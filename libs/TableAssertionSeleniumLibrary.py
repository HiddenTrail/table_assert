...
class TableAssertionSeleniumLibrary:
    ...
    def _get_table_headers(self, table_locator, header_appendix):
        ...
        library_handle = BuiltIn().get_library_instance("SeleniumLibrary")
        header_elements = library_handle.get_webelements(f"{table_locator}{header_appendix}")
    ...
    def _get_table_data(self, table_locator, row_appendix, cell_appendix):
        ...
        library_handle = BuiltIn().get_library_instance("SeleniumLibrary")
        rows = library_handle.get_webelements(f"{table_locator}{row_appendix}")
        data = []
        row_number = 0
        for row in rows:
            row_number += 1
            cell_elements = library_handle.get_webelements(f"{table_locator}{row_appendix}[{row_number}]{cell_appendix}")
